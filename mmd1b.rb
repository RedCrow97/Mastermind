class Mastermind # Class Mastermind, this is Mastermind mmd1b.rb
...
# others =  "W" White, "K" Black (for scoring)
...
# boldg(string)     "\e[1;32m#{string}\e[0m"
  ...
  def initialize # Method initialize, allocate space for objects
    ...
    4.times { arr << rand(6) } # Loop to insert 4 random numbers from 0 to 5, as characters in a string
    arr.each { |x| computer << colors[x] } # For each # in the array, replace with corresponding coded color
    # for example, if the randomly picked array "arr" is 1234, array computer gets "R","Y","G","C"
  ...
  def instructions # Method, Print Instructions to the screen
    ...
    puts "        LET'S PLAY #{boldg("MASTERMIND")}!! Guess all 4 colored pegs in 10 turns!"
    puts "     Skye-Free's Ruby version of Mastermind, fixed up a little bit -RC97"
    puts "         Note - Enter only 4 pegs, of the 6 colors from options below"
    puts "\n                             *Options*"
...
# which would have maybe enabled me to erase the input line (>>bpbr) and clear more screen or overwite it?
